module zeta
    ...
    subroutine zeta_equation()
        ...
        print*, 'Cory', cory(xi_dp, yi_dp, zi_dpst:zi_dped)
          ...
          print '("Time = ",f10.3," seconds.")', timer_ed - timer_st
        !!----------------------------------------------------------------------
    ...
    subroutine offline_curlnonl(cmode)
        ...
        !!----------------------------------------------------------------------
        ! Verifying calculation and clean up
        print*, '  '
        print*, '  Clean up and verifying offline calculation...'
        ...
        do iz = zi_dpst, zi_dped
            ...
            print*, 'Curlmet: ', curlmet(xi_dp, yi_dp, iz)
            if (allocated(err_nlsub)) then
                print*, 'err_nlsub', err_nlsub(xi_dp, yi_dp, iz)
            endif
        enddo
    endsubroutine
    ...
    subroutine decomp_curladv()
        ...
        use popfun, only : u2t
        implicit none
        real(kind=kd_r) :: WORK(nx, ny), WORKB(nx, ny), WORKTx(nx, ny), WORKTy(nx, ny), &
                           advthencur(nx, ny), curthenadv(nx, ny)
    ...
    subroutine init_zetavars_input(cmode)
        implicit none
        integer, intent(in) :: cmode

        print*, "  "
        print*, "Initializing zeta module input variables ..."

        allocate(uc(nx, ny, nz), vc(nx, ny, nz))
        uc = 0.; vc = 0.
        if (abs(cmode) == 0 .or. abs(cmode) == 1 .or. abs(cmode) == 2) then
            allocate(advx  (nx, ny, nz), advy  (nx, ny, nz), &
    ...
    subroutine init_zetavars_output(cmode)
        implicit none
        integer, intent(in) :: cmode

        print*, "  "
        print*, "Initializing zeta module output variables ..."
        ...
        if (abs(cmode) == 1 .or. abs(cmode) == 2) then
            allocate(err_nlsub(nx, ny, nz))
            err_nlsub = 0.
        endif

        if (abs(cmode) == 1 .or. abs(cmode) == 2 .or. abs(cmode) == 3) then
            allocate(curladv(nx, ny, nz), curlmet(nx, ny, nz))
            curladv = 0.; curlmet = 0.
        endif

        if (abs(cmode) == 2 .or. abs(cmode) == 3) then
            allocate(advu (nx, ny, nz), advv (nx, ny, nz), advw (nx, ny, nz), &
                     advVx(nx, ny, nz), advVy(nx, ny, nz), advVz(nx, ny, nz), err_nldecomp(nx, ny, nz))
    ...
    subroutine release_zetavars_input(cmode)
        implicit none
        integer, intent(in) :: cmode

        print*, " "
        print*, '  Deallocating zeta input variables ...'
    ...
    subroutine release_zetavars_output(cmode)
        implicit none
        integer, intent(in) :: cmode

        print*, " "
        print*, '  Deallocating zeta output variables ...'
    ...
    endsubroutine
    ! subroutine init_zetavars(cmode)
    !     implicit none
    !     integer, intent(in) :: cmode
    !
    !     print*, "  "
    ...
    ! subroutine release_zetavars(cmode)
    !     implicit none
    !     integer, intent(in) :: cmode
    !
    !     print*, " "
