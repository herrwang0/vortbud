...
!--------------------Mean/Eddy decomposition------------------------------------
    ...
    if (ifmeaneddy) then
        ...
        do iyr = 1, nyr, 1
            do isec = 1, nsec, 1
                call get_mmdd(seclist(isec, 1), trim(meanfreq), avnm_clm)
                ...
                if (iostat /= nf90_noerr) then
                    ...
                    call loadave_vars_sf(3, yrlist(iyr:iyr+slyr-1), (/ (ida, ida = seclist(isec,1), seclist(isec,2)) /))
                ...
                call init_meaneddy_vars()

                call loadave_zeta_sf(yrlist(iyr:iyr+1-1), (/ (ida, ida = seclist(isec,1), seclist(isec,2)) /))

                write(fn_zetae, '(A)') './fn_zetae_test.nc'
                call output_me(fn_zetae)
                call release_zetavars_output(2)
